...
var pool = require('../connector/mysqlConnector');

function generate_html(ship_id) {
    ...
    request({
      ...
      if (!error && response.statusCode === 200) {
        ...
        con.connect(function(err) {
          ...
          command = command.slice(0, -1);
          command += ");";
          con.query("DELETE FROM ships WHERE ship_id = '" + ship_id + "';", function(err, result) {
            if (err) throw err;
...
function update_ships_val(ship_id, colname, colval) {
  var mysql = require('mysql');
...
function generate_html(ship_id) {
  ...
  con.connect(function(err) {
    ...
    con.query(command, function(err, result) {
      ...
      var webstr = "<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><title>USS Illini</title><link rel='stylesheet' href='ships.css'></head><body><div id='container'>" + datastr + "</div></body></html>";
      console.log(webstr);
    });
  })
}
update_ships_val("4292818736","name","Quincy");
